...
public class ReferenceConfig<T> extends AbstractReferenceConfig {
    ...
    /**
     * {@url https://dubbo.apache.org/zh/docs/v2.7/dev/source/refer-service/}
     * 1. 参数校检
     * 2. 检查核心配置
     * 3. 收集配置存储在Map 中
     * 4. 处理MethedConfig 实例中的事件，比如 onreturn、onthrow、oninvoke 等。
     * 5. 创建代理对象
     */
    private void init() {
        // 避免重复初始化
    ...
    /**
     * 创建代理对象：
     * 1.1 本地引用，则生成本地代理使用 协议为injvm
     * 2.1 远程引用，点对点引用 。直接指定调用某个服务
     * 2.2 远程引用，单个注册中心或者单个服务提供者直连
     * 2.3 多个注册中心或多个服务提供者，或者两者混合
     * 3.1 创建代理
     * @param map
     * @return
     */
    ...
    private T createProxy(Map<String, String> map) {
        ...
        if (isJvmRefer) {
            ...
            if (urls.size() == 1) {
                // TODO 调用 RegistryProtocol 的 refer 构建 Invoker 实例
                ...
                // 获取所有的 Invoker
                for (URL url : urls) {
                    // TODO 通过 refprotocol 调用 refer 构建 Invoker，refprotocol 会在运行时
                    // TODO 根据 url 协议头加载指定的 Protocol 实例，并调用实例的 refer 方法
        ...
        // <dubbo:reference check="true"> 进行检查
        Boolean c = check;
